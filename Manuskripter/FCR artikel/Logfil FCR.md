...
plink \
  --allow-extra-chr \
  --allow-no-sex \
  --bfile $fcr/SNP/all_by_bcf_use_this \
  --dog \
  --double-id \
  --geno 0.05 \
  --indep-pairwise 50 10 0.5 \
  --out fcrpruned_pendleton_r2_0.05_geno_0.05 \
  --pheno $fcr/SNP/phenos/pheno_fcr_vs_eriks.txt \
  --remove $fcr/SNP/phenos/3dogs.txt
```
...
plink \
  ...
  --geno 0.05 \
  --keep phenos/pheno_fcr_vs_{both,iDog,eriks}.txt \
  --out fcr_vs_{both,iDog,eriks}_frq_g5percent.frq\
  --pheno phenos/pheno_fcr_vs_{both,iDog,eriks}.txt \
  --within phenos/pheno_fcr_vs_{both,iDog,eriks}.txt
```
...
```R
hentfil(fcr_vs_{both,iDog,eriks}_frq_g5percent.frq)
clean_plink
annotategenes
addgenes
fcr_frq_clean_ann_genes$compare<-ifelse(as.character(fcr_frq_clean_ann_genes$Allele)==as.character(fcr_frq_clean_ann_genes$A1),"same","diff")
fcr_frq_clean_ann_genes$FCR<-ifelse(fcr_frq_clean_ann_genes$compare=="same",fcr_frq_clean_ann_genes$MAF_A,1-fcr_frq_clean_ann_genes$MAF_A)
fcr_frq_clean_ann_genes$Control<-ifelse(fcr_frq_clean_ann_genes$compare=="same",fcr_frq_clean_ann_genes$MAF_U,1-fcr_frq_clean_ann_genes$MAF_U)
```
