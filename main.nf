...
markdup_java_options        [str] Establish values for markDuplicates memory consumption
    ...
    Annotation:
      ...
      --snpeff_config              [file] Specify the path to snpEff configfile (custom genomes_base)
    ...
    References:                           If not specified in the configuration file or you wish to overwrite any of the references.
      ...
      --refflat                    [file] Annotation file for CNVkit - UCSC RefFlat, BED, or GFF file
      --cnvkit_pon                 [file] file for cnvkit, typically called my_reference.cnn - produced with cnvkit.py reference
      --cnvkit_wgs                  [bool] Is it wgs?
...
params.refflat = params.genome && 'cnvkit' in tools ? params.genomes[params.genome].refflat ?: null : null
...
ch_refflat = params.refflat && 'cnvkit' in tools ? Channel.value(file(params.refflat)) : "null"
...
ch_cnvkit_pon = params.cnvkit_pon ? Channel.value(file(params.cnvkit_pon)) : "null"
...
ch_cnvkit_wgs = params.cnvkit_wgs ? Channel.value(params.cnvkit_wgs) : "null"
...
if ('cnvkit_wgs' in tools)                  summary ['cnvkit_wgs'] = params.cnvkit_wgs ? 'Yes' : 'No'
...
if (params.refflat)                 summary['refflat']                 = params.refflat
if (params.cnvkit_pon)                 summary['cnvkit_pon']                 = params.cnvkit_pon
if (params.snpeff_cache)            summary['snpEff cache']            = params.snpeff_cache
...
process MarkDuplicates {
    ...
    script:
    markdup_java_options = "'-Xmx6g -Xms4000m'"
...
// STEP CNVkit.1 - make ref from pon

//process CNVkit_ref {
//    tag "Making ref"
//
//  publishDir "reference_genome/", mode: params.publish_dir_mode
//
//    input:
//        file(fasta) from ch_fasta
//        file(refflat) from ch_refflat
//        val cnvkit_wgs from ch_cnvkit_wgs
//        file(targetBED) from ch_target_bed
//
//
//    output:
//        file("cnvkit_reference.cnn") into cnvkit_refOut //for batch
//        //set file with all (anti)target for metrics, but files aren't named
//
//    when: !(params.cnvkit_pon) && 'cnvkit' in tools 
//    if (params.cnvkit_wgs) {
//    calc_meth = "--method wgs"
//    } else if (params.target_bed){
//    calc_meth = "--targets ${targetBED}"
//    } else exit 1, "You have not specified --cnvkit_wgs or --target_bed targetBed"
...
// STEP CNVkit.2 - analysis

process CNVkit {
    tag "${idSampleTumor}"

    publishDir "${params.outdir}/VariantCalling/${idSampleTumor}_vs_pon/CNVkit", mode: params.publish_dir_mode

    input:
      set idPatient, idSampleNormal, file(bamNormal), file(baiNormal), idSampleTumor, file(bamTumor), file(baiTumor) from pairBamCNVkit
        //file(cnvkit_reference) from cnvkit_refOut
        file(cnvkit_reference) from ch_cnvkit_pon //So it can be altered if samples are trash
        val cnvkit_wgs from ch_cnvkit_wgs
        file(targetBED) from ch_target_bed
...
process Snpeff {
    ...
    """
    snpEff -Xmx${task.memory.toGiga()}g -c ${snpeff_config}\
        ${snpeffDb} \
