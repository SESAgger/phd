...
Kunne man ikke zippe bamoutput???
*/
...
s
	    ...
	    */
	    ...
	    process gVCFCombine {
	        ...
	        script:
	        """
	        java -Xmx${params.singleCPUMem}g -jar /usr/GenomeAnalysisTK.jar \
	            -T CombineGVCFs \
	            -V ${vcfs.join(' -V ')} \
	            -R $reference \
	            -o ${chrom}.vcf -L $chrom
	    ...
	    process hardfilters_snp {
	        ...
	        script:
	        """
	        java -Xmx${params.singleCPUMem}g -jar /usr/GenomeAnalysisTK.jar -T SelectVariants -R $reference \
	            -V $vcf -selectType SNP -o ${key}_raw_SNP_1.vcf
	        java -Xmx${params.singleCPUMem}g -jar /usr/GenomeAnalysisTK.jar -T VariantFiltration -R $reference -V ${key}_raw_SNP_1.vcf \
	    ...
	    process hardfilters_indel {
	        ...
	        java -Xmx${params.singleCPUMem}g -jar /usr/GenomeAnalysisTK.jar -T VariantFiltration -R $reference -V ${key}_raw_INDEL_1.vcf \
	            --filterExpression "QD < 2.0" --filterName "QD_less_than_2_filter" \
