...
print("File is called: "+args.name+"noGJ_annotated_AA_TSTV_PhyloP_SIFT.tsv")
...
## Write summary
w=open(args.name+'_No_GJ_reference_file_summary.txt','w')
w.write(
'#Summary from processing of ' + "test" +'\n'
'#Can be read as tsv with comment="#" and sep="\t"\n'
'Name\tNumber\t%of all\n'
'SNPs where an ancestral allele could be determined:\t' + str(with_AA) + '\t' + str(round(with_AA/len(ref2)*100,2)) + '%\n'
'SNPs with a transversion:\t' + str(tv) + '\t' + str(round(tv/len(ref2)*100,2)) + '%\n'
'Transversions with known ancestral allele:\t' + str(with_AA_tv)+ '\t' + str(round(with_AA_tv/len(ref2)*100,2)) + '%\n'
'SNPs with PhyloP score:\t' + str(with_PP) + '\t' + str(round(with_PP/len(ref2)*100,2)) + '%\n'
'SNPs with sift:\t' + str(with_sift) + '\t' + str(round(with_sift/len(ref2)*100,2)) + '%\n'
'SNPs with Phastcon score:\t' + str(round(with_phastcon,2)) + '\t' + str(round(with_phastcon/len(ref2)*100,2)) + '%\n'
'SNPs with all conservation scores:\t' +str(round(with_all_c,2)) + '\t' + str(round(with_all_c/len(ref2)*100,2)) + '%\n'
'Transversions with all information:\t' +str(round(with_AA_tv_PP_sift_phastcon,2)) + '\t' + str(round(with_AA_tv_PP_sift_phastcon/len(ref2)*100,2)) + '%\n'
)
w.close()